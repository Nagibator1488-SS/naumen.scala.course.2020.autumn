object Exercises {
  ...
  def primeFactor(number: Int): Seq[Int] = {
    ...
    var set : Seq[Int] = List()
    for (i <- 2 to Math.round(Math.sqrt(mutableNumber)).asInstanceOf[Int] + 1)
      while (mutableNumber % i == 0) {
        set :+= i.asInstanceOf[Int]
        mutableNumber /= i
      }
    if (mutableNumber != 1)
      set :+= mutableNumber
  ...
  def sortByHeavyweight(ballsArray: Map[String, (Int, Double)] = balls): Seq[String] = {
    ...
    val sortedNames = for (ball <- sortedMap) yield ball._1
    sortedNames
  }
}
